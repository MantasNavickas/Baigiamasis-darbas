    ...
    def add_member_tax_info(self):
        with sqlite3.connect(self.db_name) as conn: 
            ...
            try:
                ...
                results = c.execute(f"SELECT Vardas, Pavardė, Adresas FROM {self.db_table}").fetchall()

                for result in results:
                    # Gauti praeitų metų duomenis
                    debt_overpayment = c.execute(f"""
                    ...
                    """, (result[0], result[1], result[2], year_now, 0, total_debt, result[2], year_now))
